...
class Board extends CI_Controller {
	...
	public function lists() {
		...
		// 페이지네이션 설정
		$config ['base_url'] = '/test/board/lists/ci_board' . $page_url . '/page/';
		// 페이징 주소
	...
	function write() {
		...
		if (@$this->session->userdata ( 'logged_in' ) == TRUE) {
			...
			if ($this->form_validation->run () == TRUE) {
				...
				if ($result) {
					// 글 작성 성공시 게시판 목록으로
					alert ( '입력되었습니다.', '/test/board/lists/' . $this->uri->segment ( 3 ) . '/page/' . $pages );
// 					log_message('thithit', $pages);
					exit ();
				} else {
					// 글 실패시 게시판 목록으로
					alert ( '다시 입력해 주세요.', '/test/board/lists/' . $this->uri->segment ( 3 ) . '/page/' . $pages );
					exit ();
				}
			} else {
				// 쓰기폼 view 호출
			...
			alert ( '로그인후 작성하세요', '/test/auth/login/' );
			exit ();
		}
	}
	...
	function modify() {
		...
		if (@$this->session->userdata ( 'logged_in' ) == TRUE) {
			...
			if ($writer_id->user_id != $this->session->userdata ( 'username' )) {
				alert ( '본인이 작성한 글이 아닙니다.', '/test/board/view/' . $this->uri->segment ( 3 ) . '/board_id/' . $this->uri->segment ( 5 ) . '/page/' . $pages );
				exit ();
			}
			...
			if ($this->form_validation->run () == TRUE) {
				if (! $this->input->post ( 'subject', TRUE ) and ! $this->input->post ( 'contents', TRUE )) {
					...
					alert ( '비정상적인 접근입니다.', '/test/board/lists/' . $this->uri->segment ( 3 ) . '/page/' . $pages );
					exit ();
				}
				...
				if ($result) {
					// 글 작성 성공시 게시판 목록으로
					alert ( '수정되었습니다.', '/test/board/lists/' . $this->uri->segment ( 3 ) . '/page/' . $pages );
					exit ();
				} else {
					// 글 수정 실패시 글 내용으로
					alert ( '다시 수정해 주세요.', '/test/board/view/' . $this->uri->segment ( 3 ) . '/board_id/' . $this->uri->segment ( 5 ) . '/page/' . $pages );
					exit ();
				}
			} else {
				// 게시물 내용 가져오기
			...
			alert ( '로그인후 수정하세요', '/test/auth/login/' );
			exit ();
		}
	}
	...
	function delete() {
		...
		if (@$this->session->userdata ( 'logged_in' ) == TRUE) {
			...
			if ($writer_id->user_id != $this->session->userdata ( 'username' )) {
				alert ( '본인이 작성한 글이 아닙니다.', '/test/board/view/' . $this->uri->segment ( 3 ) . '/board_id/' . $this->uri->segment ( 5 ) . '/page/' . $this->uri->segment ( 7 ) );
				exit ();
			}
			...
			if ($return) {
				// 삭제가 성공한 경우
				alert ( '삭제되었습니다.', '/test/board/lists/' . $this->uri->segment ( 3 ) . '/page/' . $this->uri->segment ( 7 ) );
			} else {
				// 삭제가 실패한 경우
				alert ( '삭제 실패하였습니다.', '/test/board/view/' . $this->uri->segment ( 3 ) . '/board_id/' . $this->uri->segment ( 5 ) . '/page/' . $this->uri->segment ( 7 ) );
			}
		} else {
			alert ( '로그인후 삭제하세요', '/test/auth/login/' );
			exit ();
		}
	}
	
	/**
	 * url중 키값을 구분하여 키의 다음값을 가져오도록.
	 *
	 * @param Array $url
	 *        	: segment_explode 한 url값
	 * @param String $key
	 *        	: 가져오려는 값의 key
